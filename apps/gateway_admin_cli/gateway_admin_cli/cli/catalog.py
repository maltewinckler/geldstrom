...
from gateway_admin_cli.application.commands import SyncInstituteCatalogCommand
...
def sync_catalog(
    csv_path: Annotated[
        Path,
        typer.Argument(help="Path to the Bundesbank FinTS institute CSV file"),
    ],
) -> None:
    ...
    async def _run() -> None:
        async with build_factory() as factory:
            try:
                result = await SyncInstituteCatalogCommand.from_factory(factory)(
                    csv_path
                )
        ...
        if result.skipped_rows:
            console.print(
                f"[yellow]{len(result.skipped_rows)} rows with a valid BLZ were skipped:[/yellow]"
            )
