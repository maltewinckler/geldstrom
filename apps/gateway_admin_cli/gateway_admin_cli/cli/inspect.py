...
from gateway_admin_cli.application.queries import InspectBackendStateQuery
...
def inspect_state(
    ...
    async def _run() -> None:
        async with build_factory() as factory:
            report = await InspectBackendStateQuery.from_factory(factory)(blz=blz)
        ...
        if report.product_registration:
            reg = report.product_registration
            console.print(f"Product registration: v{reg.product_version}")
        else:
