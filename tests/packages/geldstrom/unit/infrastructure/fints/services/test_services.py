...
# --------------------------------------------------------------------------
# Port Conformance Tests — verify adapters satisfy port protocols
# --------------------------------------------------------------------------


class TestPortConformance:
    """Verify each adapter structurally satisfies its port protocol."""

    def _make_creds(self):
        creds = MagicMock()
        creds.route = BankRoute(country_code="DE", bank_code="12345678")
        return creds

    def test_account_adapter_has_port_methods(self):
        from geldstrom.infrastructure.fints.services.accounts import (
            FinTSAccountService,
        )

        adapter = FinTSAccountService(self._make_creds())
        assert callable(getattr(adapter, "fetch_accounts", None))
        assert callable(getattr(adapter, "fetch_bank_capabilities", None))

    def test_balance_adapter_has_port_methods(self):
        from geldstrom.infrastructure.fints.services.balances import (
            FinTSBalanceService,
        )

        adapter = FinTSBalanceService(self._make_creds())
        assert callable(getattr(adapter, "fetch_balances", None))
        assert callable(getattr(adapter, "fetch_balance", None))

    def test_transaction_adapter_has_port_methods(self):
        from geldstrom.infrastructure.fints.services.transactions import (
            FinTSTransactionService,
        )

        adapter = FinTSTransactionService(self._make_creds())
        assert callable(getattr(adapter, "fetch_history", None))

    def test_metadata_adapter_has_port_methods(self):
        from geldstrom.infrastructure.fints.services.metadata import (
            FinTSMetadataService,
        )

        adapter = FinTSMetadataService(self._make_creds())
        assert callable(getattr(adapter, "get_tan_methods", None))
...
def test_transactions_adapter_camt_parsing():
    """Test CAMT parsing through the transaction pipeline."""
    from geldstrom.infrastructure.fints.operations.transactions.camt import (
        _camt_to_feed,
    )
    ...
    feed = _camt_to_feed("acct-1", [document], [])
...
def test_transactions_adapter_pending_entries():
    """Test pending entry parsing through transaction pipeline."""
    from geldstrom.infrastructure.fints.operations.transactions.camt import (
        _camt_to_feed,
    )
    ...
    feed = _camt_to_feed("acct-2", [], [document])
...
def test_accounts_adapter_capabilities_from_operations():
    ...
    from geldstrom.infrastructure.fints.services.accounts import FinTSAccountService

    creds = MagicMock()
    adapter = FinTSAccountService(creds)
...
def test_accounts_adapter_discovers_accounts_and_capabilities_from_context():
    """Test single-dialog discovery result through accounts adapter."""
    ...
    adapter = FinTSAccountService(creds)
    ctx = MagicMock()
    ctx.parameters.bpd.get_supported_operations.return_value = {
        "GET_BALANCE": True,
    ...
    account_info = AccountInfo(
        ...
        allowed_operations=["HKSAL"],
    )

    with pytest.MonkeyPatch.context() as monkeypatch:
        mock_ops = MagicMock()
        mock_ops.fetch_all.return_value = [account_info]
        monkeypatch.setattr(
            "geldstrom.infrastructure.fints.operations.AccountOperations",
            lambda dialog, parameters: mock_ops,
        )

        discovery = adapter.discover_from_context(ctx)

    assert len(discovery.accounts) == 1
    assert discovery.accounts[0].account_id == "123456:0"
    assert discovery.capabilities.supported_operations == frozenset({"GET_BALANCE"})
...
def test_accounts_adapter_merges_sepa_metadata():
    ...
    upd_accounts = [
        AccountInfo(
            ...
            subaccount_number="0",
            iban="DE001234",
            bic="BANKDEFFXXX",  # Already merged by fetch_all()/_join_sepa_info()
            currency="EUR",
            owner_name=["Alice"],
            product_name="Checking",
            account_type=1,
    ...
    accounts = adapter.map_accounts_from_operations(default_route, upd_accounts)
...
def test_balance_adapter_operations_parsing():
    ...
    from geldstrom.infrastructure.fints.operations import BalanceResult, HisalBalance
    from geldstrom.infrastructure.fints.services.balances import FinTSBalanceService

    creds = MagicMock()
    adapter = FinTSBalanceService(creds)
...
def test_transactions_adapter_mt940_parsing():
    """Test MT940 transaction parsing through transaction pipeline."""
    from geldstrom.infrastructure.fints.operations.transactions.mt940 import (
        _transactions_to_feed,
    )
    ...
    feed = _transactions_to_feed("acct", txns)
...
def test_transactions_adapter_has_more_propagation_mt940():
    ...
    feed = _transactions_to_feed("acct", txns, has_more=False)
    assert feed.has_more is False

    # Test with has_more=True
    feed = _transactions_to_feed("acct", txns, has_more=True)
...
def test_transactions_adapter_has_more_propagation_camt():
    ...
    # Test with has_more=False
    feed = _camt_to_feed("acct", [document], [], has_more=False)
    assert feed.has_more is False

    # Test with has_more=True
    feed = _camt_to_feed("acct", [document], [], has_more=True)
    assert feed.has_more is True
...
def test_transactions_adapter_camt_counterpart_name_structures(
    ...
    feed = _camt_to_feed("acct", [document], [])
